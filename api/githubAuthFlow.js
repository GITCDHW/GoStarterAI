...
/**
 * Helper: Create or update a file in repo
 */
const pushFile = async (accessToken, repoOwner, repoName, filePath, fileContent, commitMessage) => {
  ...
  try {
    ...
    sha = res.data.sha; // existing file sha
  } catch (err) {
    if (err.response?.status !== 404) throw err; // only ignore 404
  }
...
/**
 * Push project files to repo (index.html + workflow)
 */
...
jobs:
  deploy:
    ...
    await pushFile(
      ...
      workflowPath,
      workflowContent,
      'Add or update GitHub Actions workflow for deployment'
    );
    ...
    console.error('❌ Error pushing files:', message);
...
export default async function handler(req, res) {
  ...
  if (!pushResult.success) {
    console.error('Failed after retries:', pushResult.error);
    return res.status(500).json({
      error: `Repository created, but code push failed: ${pushResult.error}`
    });
  }
