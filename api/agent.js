...
async function generateCode(prompt,name) {
      // Select the model
    const masterPrompt = `you are a professional frontend developer,your goal is to provide an unique(it MUST NOT LOOK GENERIC) clean,modern efficient,well formatted html code for a **responsive** **mobile friendly** landing page,for the given users request: ${prompt},with the business name:${name},make educated guesses,if user doesnt provides enough information,and strictly use material UI framework,dont write any custom css dont add any other text or code delimiters,just plain text,MAKE SURE THE ENTIRE CODE WORKS`
